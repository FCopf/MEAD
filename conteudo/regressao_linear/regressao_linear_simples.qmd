...
description: "Modelos de regressão linear simples, ANOVA da regressão, coeficientes de determinação e diagnósticos da regressão."
Categories: ["Regressão Linear", "Estatística Descritiva", "Modelagem Matemática", "Análise de Variância"]

image: "images/regressao_linear_simples.jpg"
execute:
  echo: true
  warning: false
  include: true
...
```{r}
#| code-fold: true
st <- read_csv("https://github.com/FCopf/datasets/raw/refs/heads/master/HubbardBrook_wide.csv") |>
...
```{r}
rk = read_csv('https://github.com/FCopf/datasets/raw/refs/heads/master/RIKZ.csv')
rks = rk |> 
  (\(df) df[seq(3,43,by = 5),])()
```
...
gdfa <- ggplot(DF) +
  ...
  annotate("text", x = 6.5, y = 1, size = 4, label = bquote(gl[numerador]==.(num1) ~ ";" ~ gl[denominador]==.(den1))) +
  annotate("text", x = 6.5, y = 0.85, size = 4, label = bquote(gl[numerador]==.(num2) ~ ";" ~ gl[denominador]==.(den2))) +
