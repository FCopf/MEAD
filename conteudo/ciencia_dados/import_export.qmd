...
description: "Técnicas de importação e exportação de dados usando o pacote `readr` do Tidyverse."
categories: ["Ciência de Dados", "R", "Tidyverse", "Importação/Exportação de dados"]
...
Carrege o pacote ```readr``` e importe o arquivo ```res.csv``` disponível no repositório [datasets](https://github.com/FCopf/datasets) do <i class="bi bi-github"></i>. É possível importar o arquivo diretamente do repositório:

```{r}
library(readr)
res = read_delim(file = "https://github.com/FCopf/datasets/raw/refs/heads/master/Reservatorios_Parana_parcial.csv",
                  delim = ',',
...
::: {.callout-note}
Se optar por fazer o download do arquivo, basta acessar pelo link ([res](https://github.com/FCopf/datasets/blob/master/Reservatorios_Parana_parcial.csv)), salvá-lo em seu diretório de trabalho e importar com o comando:

```{r}
#| eval: false
res = read_delim(file = "res.csv", delim = ",")
```
