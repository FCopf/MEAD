...
Carregar os pacote `dplyr`, `tidyr` e `readr``
...
```{r}
regiao <- read_csv("https://raw.githubusercontent.com/FCopf/datasets/master/regiao.csv")
habitat <- read_csv("https://raw.githubusercontent.com/FCopf/datasets/master/habitat.csv")
regiao
habitat
```

#### Tabela regiao {.unnumbered}: Contém informações sobre a bacia hidrográfica, área da bacia e município de alguns riachos da região litorânea de São Paulo.

#### Tabela habitat  {.unnumbered}: Contém informações sobre a largura e profundidade desses riachos. Algumas entradas são comuns às duas tabelas, enquanto outras são exclusivas de uma delas. A coluna `Riacho` serve como chave para combinar as informações.
...
```{r}
hbrook <- read_csv("https://raw.githubusercontent.com/FCopf/datasets/master/HubbardBrook.csv")
hbrook
```
