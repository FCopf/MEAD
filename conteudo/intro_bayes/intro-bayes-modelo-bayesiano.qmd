...
::: {.callout-tip title="Explore o efeito da priori informativa!"}
...
👉 [**Abrir o app: Inferência Bayesiana (Shiny)**](https://fcopf-binomial-bayesiana.share.connect.posit.cloud/){target=_blank}

✅ **Dicas:**

1. **Defina o número total de observações ($N$)** usando o controle deslizante no painel lateral.

2. **Escolha o número de sucessos ($k$)** observados entre essas $N$ observações.

3. **Ajuste os parâmetros da distribuição *a priori* Beta**, modificando os valores de $\alpha$ e $\beta$ conforme sua hipótese inicial.

4. **Observe os gráficos e compare as distribuições** *a priori* (vermelho), o perfil de verossimilhança binomial (verde) e a distribuição *a posteriori*, resultante da combinação das outras duas (azul).
